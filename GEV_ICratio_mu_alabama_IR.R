...
gev.ratio.ic.mu=function(xp,t0,t1,y.fit,ydat,ci.p=0.95,like.num = 1000 ,mulink=identity, siglink = identity, show = TRUE,  method="Nelder-Mead",maxit = 10000,to.plot=FALSE, ...){
	...
	gev.lik <- function(par) {
     ...
     #      profil_o=lapply(ratio.l,optimiser_profil,xp=xp)
     ...
     #      r=uniroot.all(f_roots,c(p0p1,1))
     #      lr=uniroot.all(f_roots,c(0,p0p1))
     #      debug(binf_time)
     ic_inf=binf_time(c(-0.1,p0p1),fun=f_roots,nbdiv=2,xmax=p0p1)
     ratio.l=ic_inf$x_vrais
     parmax=ic_inf$y_vrais
     browser()
     sr=seek_roots(c(-0.1,1),fun=f_roots,nbdiv=10,xmax=p0p1)
     ratio.l=sr$x_vrais
     parmax=sr$y_vrais
     summary(parmax)
     o.ratio.l=order(ratio.l)
     ratio.l=ratio.l[o.ratio.l]
     parmax=parmax[o.ratio.l]
     browser()
