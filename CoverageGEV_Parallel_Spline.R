...
iFARplot <- function() {
	...
	covariate=(mapply(qgev,mu=mu,sigma=sigma,xi=shape,MoreArgs=list("p"=0.5)))
	covariate=predict(rq( y~ bs(year, df=5,degree=5), tau=0.5))
	...
	updatePlot <- function(h,...){
		...
		with(df.plot,lines(year,mua,col="red"))
	...
	ComputeFAR <- function(h,...){
		...
		boot.res=boot(data=df.plot,statistic=FARBoot.Spline,R=250,p1=c(t1,xp),x2=t0,parallel="snow")
